...
namespace ConsoleComplete
{
	class MainClass
	{
		...
		public static void Main(string[] args)
		{
			...
			{
				var balance = web3.Eth.GetBalance.SendRequestAsync(accounts[0]).Result;
			...
			String txHash = web3.Eth.DeployContract.SendRequestAsync(bytes, accounts[0], new Nethereum.Hex.HexTypes.HexBigInteger(1000000)) .Result;
			...
			var receipt = web3.Eth.Transactions.GetTransactionReceipt.SendRequestAsync(txHash).Result;
			Console.Write("Processing");

			while (receipt == null)
			{
				System.Threading.Thread.Sleep(3000);
				receipt = web3.Eth.Transactions.GetTransactionReceipt.SendRequestAsync(txHash).Result;
				Console.Write(".");
			}
			...
			var abi = GetABIFromFile(CONTRACT_PATH + CONTRACT_FILE_NAME + ".abi"); //@"[{""constant"":false,""inputs"":[{""name"":""x"",""type"":""uint8""},{""name"":""y"",""type"":""uint8""}],""name"":""multiply"",""outputs"":[{""name"":""product"",""type"":""uint8""}],""payable"":false,""type"":""function""}]"; //GetABIFromFile(CONTRACT_PATH + "math.abi"); //
			...
			{
				var estimatedTx = lodgeFunction.SendTransactionAsync(accounts[1]).Result;
